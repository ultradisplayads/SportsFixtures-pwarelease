...
export async function getMeasurementOverview(): Promise<MeasurementOverview> {
  const [totals, discoverClicks, conversions, venueIntent, stalePages] =
    await Promise.all([
      fetchOrganicAndSessionTotals(),
      fetchDiscoverClicks(),
      fetchConversionRows(),
  ...
  const conversionTotal = conversions.reduce((sum, row) => sum + row.count, 0)
  const venueIntentSessions = venueIntent.reduce(
