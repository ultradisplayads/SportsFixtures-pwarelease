...
export default function PoolLeaguesPage() {
  const [leagues] = useState<PoolLeague[]>([
    {
      id: "1",
      name: "Bangkok Pool League - Division 1",
      venue: "Sports Corner Bar",
      location: "Bangkok, Thailand",
      teams: 12,
      season: "2024 Spring",
      nextMatch: "Tonight 7:00 PM",
    },
    {
      id: "2",
      name: "Thailand Pool Championship",
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      <div className="sticky top-0 z-10 border-b border-border bg-card px-4 py-4">
        <h1 className="text-xl font-bold">Pool Leagues & Tables</h1>
        <p className="text-sm text-muted-foreground">Local pool competitions and standings</p>
      </div>
      ...
      <div className="flex flex-col gap-4 p-4">
        {leagues.map((league) => (
          <Link
            key={league.id}
            href={`/local-leagues/pool/${league.id}`}
        ...
        <div className="mt-6 rounded-lg border-2 border-primary bg-primary/5 p-4">
          <h3 className="font-semibold">Run a Pool League?</h3>
